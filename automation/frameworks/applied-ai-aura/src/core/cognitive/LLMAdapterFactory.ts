 ...
 * Creates the correct adapter based on the provider configured in .env.
 * Pattern: Factory + Strategy — each provider implements AIAdapter.
 *
 * The rest of the framework only depends on AIAdapter.
 */
...
/**
 * Creates the correct LLM adapter from .env configuration.
 * Consumers use the AIAdapter contract without provider awareness.
 */
...
/** Creates an OpenAI adapter directly (backward compatibility). */
