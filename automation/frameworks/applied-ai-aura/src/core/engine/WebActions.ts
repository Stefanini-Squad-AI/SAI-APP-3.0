/**
 * AURA — WebActions
 * Web interaction engine. AURA-native equivalent to common WebActions patterns
 * without Stagehand or external wrappers.
 *
 * Responsibilities:
 *  - Visual stabilization to prevent blank-frame interactions
 *  - Element highlight for visual debugging
...
const TIMEOUT_VISUAL_LOAD   = 150_000; // 2.5 min (heavy pages / SPA)
const TIMEOUT_ELEMENT_READY =  30_000; // 30 s default per element
const BUFFER_NAVIGATION     =   5_000; // post-navigate painting buffer
const BUFFER_ACTION         =   1_500; // post-action micro-buffer
const BUFFER_MINI            =    500; // micro wait after actions
...
export class WebActions {
  ...
  /**
   * Navigates to a URL and waits for complete visual stabilization.
   * Prevents black screenshots and interactions on partially rendered DOM.
   */
  ...
  /**
   * Clicks an element using AURA semantic resolution.
   * Target examples: "button:Login", "[aria=Submit]", "data-testid", free text.
   */
  ...
  /**
   * Types text into a field using semantic resolution.
   * Clears the field before typing (safe default behavior).
   */
  ...
  /**
   * Types character by character (useful for autocomplete fields).
   */
  async typeSlowly(
    target: string,
    value: string,
  ...
  /** Clears and empties an input field. */
  ...
  /**
   * Waits for COMPLETE page visual stabilization.
   * Combines networkidle + domcontentloaded + visible body validation.
   * Helps prevent black screenshots on SPA/lazy-loading apps.
   */
  async waitForVisualLoad(buffer = BUFFER_NAVIGATION): Promise<void> {
    console.info('[AURA/WebActions] Waiting for full visual load...');

    // 1. Network idle (critical for SPAs waiting for APIs)
    await this.page
    ...
    // 3. Ensure body has visible content
    await this.page
      .waitForFunction(
        () => {
  ...
  /**
   * Waits for ONE specific element to be stable with valid dimensions.
   * Lighter than waitForVisualLoad — use for individual elements.
   */
  async waitForElement(
    ...
    // Ensure element has real dimensions (not just recently toggled visible)
    await this.page
      .waitForFunction(
        ([sel]: [string]) => {
  ...
  /** Waits for light visual stabilization (DOM ready + buffer). */
  ...
  /** Waits until URL contains a specific string. */
  ...
  /**
   * Highlights an element before interaction.
   * Useful for debugging and cleaner report screenshots.
   */
  ...
  /**
   * Captures a screenshot after a stabilization wait.
   * Helps avoid black-frame captures.
   */
  ...
  /**
   * Captures a screenshot as base64 (for HTML report embedding).
   */
  ...
  /**
   * Resolves a target into a Playwright Locator.
   * Supports CSS selectors, XPath, text, and AURA attributes.
   */
