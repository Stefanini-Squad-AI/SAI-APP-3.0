...
/**
 * Test de cambio de idioma a portugués en la vista Home (TCO-11)
 *
 * Ejecuta language-pt.feature:
 * 1. Accede a http://18.217.121.166:92/
 * 2. Presiona el dropdown de idioma (al lado del botón de login de admin)
 * 3. Selecciona portugués
 * 4. Verifica títulos: Sobre Nós, Nossos Serviços, Visite-nos
 */
export class LanguagePtTest {
  async execute(): Promise<void> {
    ...
    const reportGenerator = new ReportGenerator(client, "LanguagePtTest");
    ...
    try {
      const featurePath = path.join(__dirname, "../features/language-pt.feature");
