 ...
 * Includes: changes, improvements, added tests, coverage, statistics and more.
 *
...
const KIND_CFG: Record<ChangeKind, { label: string; icon: string; color: string }> = {
  feature:  { label: 'Nueva Funcionalidad', icon: 'bi-rocket-takeoff', color: '#6366f1' },
  fix:      { label: 'Corrección',          icon: 'bi-bug',            color: '#ef4444' },
  refactor: { label: 'Refactorización',     icon: 'bi-tools',          color: '#8b5cf6' },
  breaking: { label: 'Cambio Disruptivo',   icon: 'bi-exclamation-triangle', color: '#dc2626' },
  docs:     { label: 'Documentación',       icon: 'bi-file-text',      color: '#22c55e' },
  perf:     { label: 'Rendimiento',         icon: 'bi-lightning-charge', color: '#eab308' },
};
...
export class ConfluenceExporter {
  ...
  render(entries: readonly ChangeEntry[]): string {
    ...
    const genDate  = new Date().toLocaleString('es-CO', { dateStyle: 'long', timeStyle: 'short' });
...
<header class="bg-zinc-900 border-b border-zinc-800 sticky top-0 z-50 backdrop-blur">
  <div class="container mx-auto px-4 max-w-7xl py-3 flex items-center justify-between gap-4 flex-wrap">
    ...
    <div class="flex items-center gap-4 text-xs text-zinc-500">
      <span><i class="bi bi-collection mr-1"></i>${total} registros</span>
...
<aside class="w-56 shrink-0 hidden lg:block">
  <div class="sticky top-20 flex flex-col gap-4">
    <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
      <div class="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">Versiones</div>
      <nav class="flex flex-col gap-0.5">${versionLinks}</nav>
    </div>
    <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
      <div class="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">Por Tipo</div>
      ${kindItems}
    </div>
  </div>
</aside>`;
  ...
  private renderStatsPanel(
    ...
    const kpis = [
      { label: 'Total Cambios',  value: entries.length, icon: 'bi-collection',    color: '#6366f1' },
      ...
      { label: 'Versiones',      value: versionCount,   icon: 'bi-tag',            color: '#8b5cf6' },
      { label: 'Autores',        value: authorCount,    icon: 'bi-people',          color: '#eab308' },
      { label: 'Último Cambio',  value: lastChange,     icon: 'bi-clock-history',  color: '#6b7280' },
    ];
...
<div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 mb-6">
  <h3 class="font-semibold text-sm text-zinc-300 mb-4">
    <i class="bi bi-pie-chart mr-2 text-indigo-500"></i>Distribución por Tipo de Cambio
  </h3>
...
<div class="flex items-center gap-2 flex-wrap mb-4">
  <span class="text-xs text-zinc-500 mr-1">Filtrar:</span>
  ${btns}
</div>`;
  }
...
<section id="${anchorId}" class="version-section mb-10">
  <div class="flex items-center gap-3 mb-4 ml-8">
    ...
    <span class="text-xs text-zinc-500 bg-zinc-800 px-2 py-1 rounded-md font-mono">${entries.length} cambios</span>
  </div>
