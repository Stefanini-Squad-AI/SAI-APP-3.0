...
export class AuraReportCollector {
  ...
  startStep(keyword: string, text: string): void {
    ...
    this.addLog('INFO', `Iniciando paso: ${keyword} ${text}`, text, keyword);
  }
  ...
  addIntentResults(intents: readonly IntentResult[]): void {
    for (const ir of intents) {
      ...
      if (ir.success) {
        this.addLog(
          'SUCCESS',
          `${ir.action} completado exitosamente`,
          ir.target,
          ir.action,
          `Selector: ${selector} · Duración: ${ir.durationMs}ms`,
        );
      } else {
        this.addLog(
          'ERROR',
          `${ir.action} FALLIDO: ${ir.error ?? 'Error desconocido'}`,
          ir.target,
          ir.action,
          `Selector: ${selector} · Duración: ${ir.durationMs}ms · Error: ${ir.error ?? ''}`,
        );
      }
    }
  }
  ...
  async endStep(
    ...
    if (this.preActionScreenshot) {
      ...
      this.preActionScreenshot = undefined;
      this.addLog('SUCCESS', `Screenshot pre-acción capturado`, undefined, 'screenshot');
      ...
      try {
        ...
        this.addLog('SUCCESS', `Screenshot capturado: ${fileName}`, undefined, 'screenshot');
      } catch {
        this.addLog('WARNING', 'No se pudo capturar screenshot para este paso');
      }
    }

    if (status === 'passed') {
      this.addLog('SUCCESS', `Paso completado exitosamente`, text, keyword, `Duración: ${durationMs}ms`);
    } else if (status === 'failed') {
      this.overallStatus = 'FAILED';
      this.addLog('ERROR', `Paso FALLIDO: ${error ?? 'Error desconocido'}`, text, keyword, error);
    }
  ...
  private async generateExecutiveSummary(data: AuraReportData): Promise<string> {
    try {
      ...
      const lang = (process.env.AURA_REPORT_LANGUAGE || 'en').toLowerCase();
      const langInstruction =
        lang === 'es' ? 'Write in Latin American Spanish.' :
        lang === 'pt' ? 'Write in Brazilian Portuguese.' :
        'Write in English.';

      const systemPrompt = `You are a senior QA manager writing executive test summaries for stakeholders.
Write a CONCISE executive summary (3-5 paragraphs). ${langInstruction}
