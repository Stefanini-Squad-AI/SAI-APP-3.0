 ...
 * Genera reportes HTML self-contained con TailwindCSS + Chart.js + Bootstrap Icons.
 * Naming: prueba-{testName}-{YYYYMMDD}-v{version}.html
 *
 * Features:
 *  - Dashboard con KPIs animados
 *  - Gráfico donut de pass-rate (Chart.js)
 *  - Timeline de escenarios con steps colapsables
 *  - Screenshots embebidos en base64
 *  - Sección de trazas de intents (debug)
...
export function buildReportFileName(testName: string, version?: string): string {
  ...
  return `prueba-${safeName}-${datePart}-v${versionPart}.html`;
}

function resolveVersion(outputDir: string, testName: string): string {
  const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
  const safeName = testName.replace(/[^a-zA-Z0-9_-]/g, '_').toLowerCase();
  const prefix = `prueba-${safeName}-${date}-v`;
...
export class TailwindReportEngine {
  ...
  private renderHTML(summary: ReportSummary, version: string): string {
    ...
    const genDate = new Date(summary.generatedAt).toLocaleString('es-CO', {
      dateStyle: 'long', timeStyle: 'short',
    });
    const duration = this.fmtDuration(summary.durationMs);
    const passColor = summary.failed > 0 ? '#ef4444' : '#22c55e';
    const passLabel = summary.failed > 0 ? 'FALLIDO' : 'EXITOSO';
...
<nav class="${bg} border-b backdrop-blur">
  <div class="container mx-auto px-4 max-w-7xl">
    <div class="flex gap-1">
      <button onclick="switchTab('summary')" id="btn-summary"
        ...
        <i class="bi bi-grid-3x3-gap mr-1.5"></i>Resumen
      </button>
      <button onclick="switchTab('scenarios')" id="btn-scenarios"
        ...
        <i class="bi bi-list-check mr-1.5"></i>Escenarios
      </button>
  ...
  private renderKPIs(s: ReportSummary, isDark: boolean): string {
    ...
    const kpis = [
      ...
      { label: 'Exitosos', value: s.passed,          icon: 'bi-check-circle',  color: '#22c55e' },
      { label: 'Fallidos', value: s.failed,           icon: 'bi-x-circle',     color: '#ef4444' },
      { label: 'Saltados', value: s.skipped + s.pending, icon: 'bi-skip-forward', color: '#6b7280' },
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
  <div class="${cardBg} border rounded-2xl p-6">
    <h3 class="font-semibold mb-4 text-sm ${isDark ? 'text-zinc-300' : 'text-slate-700'}">
      <i class="bi bi-pie-chart mr-2 text-indigo-500"></i>Distribución de Resultados
    </h3>
  ...
  <div class="${cardBg} border rounded-2xl p-6">
    <h3 class="font-semibold mb-4 text-sm ${isDark ? 'text-zinc-300' : 'text-slate-700'}">
      <i class="bi bi-bar-chart mr-2 text-indigo-500"></i>Resultados por Feature
    </h3>
  ...
  private renderScenarios(scenarios: readonly ScenarioResult[], isDark: boolean): string {
    ...
    if (scenarios.length === 0) {
      return `<div class="text-center py-20 ${isDark ? 'text-zinc-500' : 'text-slate-400'}">
        <i class="bi bi-inbox text-5xl block mb-3"></i>No hay escenarios registrados.
      </div>`;
    }
...
<div class="mb-4 flex items-center justify-between">
  <h2 class="font-bold text-sm ${isDark ? 'text-zinc-300' : 'text-slate-600'}">
    <i class="bi bi-list-check mr-2 text-indigo-500"></i>${scenarios.length} Escenarios
  </h2>
</div>
...
<table class="w-full text-sm">
  <thead>
    <tr class="${isDark ? 'bg-zinc-950/50' : 'bg-slate-50'}">
      ...
      <th class="px-4 py-2 text-left text-xs font-semibold ${isDark ? 'text-zinc-500' : 'text-slate-400'} uppercase tracking-wide">Descripción</th>
      <th class="px-4 py-2 text-right text-xs font-semibold ${isDark ? 'text-zinc-500' : 'text-slate-400'} uppercase tracking-wide">Duración</th>
    </tr>
  </thead>
  ...
  private renderChangelog(s: ReportSummary, _isDark: boolean): string {
    ...
    if (s.changelog.length === 0) {
      return `<div class="text-center py-20 ${isDark ? 'text-zinc-500' : 'text-slate-400'}">
        ...
        No hay cambios registrados. Usa <code class="font-mono text-sm">@AuraChange</code> para documentar cambios.
      </div>`;
    }
...
<div class="${cardBg} border rounded-2xl overflow-hidden">
  <div class="px-6 py-4 ${isDark ? 'border-zinc-800' : 'border-slate-200'} border-b flex items-center justify-between">
    <h3 class="font-semibold text-sm">
      <i class="bi bi-clock-history mr-2 text-indigo-500"></i>Historial de Cambios
    </h3>
    <span class="text-xs ${isDark ? 'text-zinc-500' : 'text-slate-400'}">${s.changelog.length} registros</span>
  </div>
  <div class="overflow-x-auto">
    <table class="w-full text-sm">
      <thead class="${isDark ? 'bg-zinc-950/40' : 'bg-slate-50'}">
        <tr>
          ${['Tipo','Cambio','Descripción','Versión','Autor','Fecha','Tags'].map((h) =>
...
<footer class="${isDark ? 'bg-zinc-900 border-zinc-800 text-zinc-500' : 'bg-white border-slate-200 text-slate-400'} border-t mt-8 py-4">
  <div class="container mx-auto px-4 max-w-7xl flex items-center justify-between flex-wrap gap-2 text-xs">
    ...
    <span>${s.totalScenarios} escenarios · ${s.passed} exitosos · Generado: ${new Date(s.generatedAt).toLocaleString()}</span>
  </div>
</footer>`;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (donut) {
    new Chart(donut, {
      type: 'doughnut',
      data: {
        labels: ['Exitosos','Fallidos','Saltados','Pendientes'],
  ...
  if (bar && d.featureLabels.length > 0) {
    new Chart(bar, {
      ...
      data: {
        ...
        datasets: [
          { label: 'Exitosos', data: d.featurePassed, backgroundColor: '#22c55e99', borderRadius: 4 },
          { label: 'Fallidos', data: d.featureFailed, backgroundColor: '#ef444499', borderRadius: 4 },
          { label: 'Saltados', data: d.featureSkipped, backgroundColor: '#6b728099', borderRadius: 4 },
        ]
      },
      options: {
