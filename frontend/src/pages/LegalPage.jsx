      ...
      <section className="py-16">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="bg-white rounded-2xl shadow-soft p-8 md:p-12 space-y-8">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-3">{t('legalPage.s4title')}</h2>
              <p className="text-gray-600 leading-relaxed">{t('legalPage.s4body')}</p>
            </div>

            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-3">{t('legalPage.s5title')}</h2>
              <p className="text-gray-600 leading-relaxed">{t('legalPage.s5body')}</p>
            </div>

            {/* Related documents */}
            <div className="border-t border-gray-200 pt-6">
              <p className="text-gray-700 font-semibold mb-3">{t('legalPage.relatedDocs')}:</p>
              <div className="flex flex-wrap gap-3">
                <Link to="/privacy" className="text-primary-600 hover:text-primary-700 font-medium underline underline-offset-2">
                  {t('legalPage.privacyPolicy')}
                </Link>
                <span className="text-gray-400">·</span>
                <Link to="/terms" className="text-primary-600 hover:text-primary-700 font-medium underline underline-offset-2">
                  {t('legalPage.termsConditions')}
                </Link>
              </div>
            </div>
